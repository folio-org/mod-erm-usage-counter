...
public class CsvHeaderToReportHeader {
  ...
  public static SUSHIReportHeader parseHeader(Map<String, String> headerColumns) {
    ...
    sushiReportHeader
        .setReportName(headerColumns.get(SUSHIReportHeader.SERIALIZED_NAME_REPORT_NAME));
    ...
    String reportAttrs = headerColumns
        .getOrDefault(SUSHIReportHeader.SERIALIZED_NAME_REPORT_ATTRIBUTES, "");
    String[] splitted = reportAttrs == null ? new String[0] : reportAttrs.split(";");
    List<SUSHIReportHeaderReportAttributes> headerReportAttributes = Stream
        .of(splitted)
        .map(s -> {
          String[] split = s.split("=");
          SUSHIReportHeaderReportAttributes attributes = new SUSHIReportHeaderReportAttributes();
    ...
    String errors = headerColumns
        .getOrDefault(SUSHIReportHeader.SERIALIZED_NAME_EXCEPTIONS, "");
    String[] splittedErrors = errors == null ? new String[0] : errors.split(";");
    List<SUSHIErrorModel> errorModels = Stream
        .of(splittedErrors)
        .map(s -> {
