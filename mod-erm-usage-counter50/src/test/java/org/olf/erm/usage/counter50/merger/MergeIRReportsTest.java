...
public class MergeIRReportsTest extends MergerTest<COUNTERItemReport> {
  ...
  public void testMergeReports() throws IOException, Counter5UtilsException {
    ...
    COUNTERItemReport mergedReports =
    ...
    for (int i = 0; i < mergedReports.getReportItems().size(); i++) {
      ...
      COUNTERItemUsage expectedCounterItemUsage =
          repExpected.getReportItems().stream()
              .filter(item -> item.getItem().equals(counterItemUsage.getItem()))
              .findFirst()
              .get();
      assertThat(counterItemUsage)
          .usingRecursiveComparison()
          .ignoringCollectionOrder()
